...
optparser.add_option(
    "-x", "--special", action="store_true", default=False,
    help="Special need for only output word sequence"
)
optparser.add_option(
...
parameters['special'] = opts.special
...
if parameters['special']:
    train_out = open("train.data", "w")
    for raw_sentence, data in zip(train_sentences, train_data):
    ...
    for raw_sentence, data in zip(test_sentences, test_data):
        ...
        y_preds = f_eval(*input)
        test_out.write("%d\n" % len(raw_sentence))
        test_out.writelines([ "%s\n" % " ".join(map(str, ys)) for ys in y_preds])
        test_out.writelines([ "-1\n" for i in range(len(raw_sentence)) ])
        test_ans.write("%d\n" % len(raw_sentence))
        test_ans.writelines([ "%s\n" % x[1] for x in raw_sentence ])
